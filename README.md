...
### Правила доступа к отзывам (отображение/создание)
- Требуется авторизация
- Бесплатные циферблаты: отзыв можно оставить, если нажата кнопка «Установить» (создаётся запись в `watchface_sales`) или есть запись установки (`watchface_installations`)
- Платные циферблаты: отзыв можно оставить, если есть покупка (`watchface_sales`)
- Endpoint получения `/watchface/{id}/ratings` публичный, но токен учитывается для определения `can_review`
